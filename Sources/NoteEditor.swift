...
final class TaskTextView: NSTextView {
    ...
    override func keyDown(with event: NSEvent) {
        // Cmd+D or Opt+Space to toggle task status
        if (event.modifierFlags.contains(.command) && (event.charactersIgnoringModifiers == "d" || event.charactersIgnoringModifiers == "D")) ||
           (event.modifierFlags.contains(.option) && event.keyCode == 49) {
            toggleTaskAtCurrentLine()
            return
        }
        super.keyDown(with: event)
    }

    private func toggleTaskAtCurrentLine() {
        guard let storage = textStorage else { return }
        let ns = string as NSString
        guard ns.length > 0 else {
            if shouldChangeText(in: NSRange(location: 0, length: 0), replacementString: Tasks.openPrefix) {
                storage.replaceCharacters(in: NSRange(location: 0, length: 0), with: Tasks.openPrefix)
                didChangeText()
            }
            return
        }
        ...
        if Tasks.isTask(trimmed) {
            ...
            if marker == Tasks.open {
                let markerRange = NSRange(location: line.location + indent.count, length: 1)
                if shouldChangeText(in: markerRange, replacementString: String(Tasks.done)) {
                    storage.replaceCharacters(in: markerRange, with: String(Tasks.done))
                    didChangeText()
                }
            } else {
            ...
            let insertRange = NSRange(location: line.location + indent.count, length: 0)
            if shouldChangeText(in: insertRange, replacementString: Tasks.openPrefix) {
                storage.replaceCharacters(in: insertRange, with: Tasks.openPrefix)
                didChangeText()
            }
        }
